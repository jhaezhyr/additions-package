...
public struct PairPermutationSequence<S: Sequence>: IteratorProtocol, Sequence
{
	...
	var firstIterator: IndexingIterator<[EnumeratedSequence<S>.Element]>
	...
	public init(for source: S, includingIdentityPairs: Bool = false)
	{
		...
		if includeIdentityPairs {
            self.firstIterator = source.enumerated().dropLast(0).makeIterator()
        }
		else
			{ self.firstIterator = source.enumerated().dropLast().makeIterator() }
