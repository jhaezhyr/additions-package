...
/// A struct representing an angle by wrapping its radian value (a `CGFloat`).  Provides calculations and initializations appropriate to an angle, such as degrees and radians, reference angle, and the angle between intervals like [0, 2π) and (-π, π].
public struct Angle: ExpressibleByFloatLiteral, ExpressibleByIntegerLiteral, CustomStringConvertible, Hashable, Comparable, ArithmeticSummable, ArithmeticNegatable, ArithmeticScalable, UniformRandom, Codable// , SignedNumeric
{
	public typealias ArithmeticScalar = Double
	
	public typealias Raw = CGFloat
	...
	public var nauticalAngle:	(degrees: Int, minutes: Int, seconds: Raw)
	{
		get
		{
			...
			let givenDegrees	= floor(totalSeconds.truncatingRemainder(dividingBy: (60 * 60)));	totalSeconds -= givenDegrees * (60 * 60)
			let givenMinutes	= floor(totalSeconds.truncatingRemainder(dividingBy: 60));			totalSeconds -= givenMinutes * 60
	...
	{
		let raw = atan(facing.y / facing.x) as CGFloat
...
public extension Angle {
    static var leastNormalMagnitude: Angle { return Angle(Raw.leastNormalMagnitude) }
    static var greatestFiniteMagnitude: Angle { return Angle(Raw.greatestFiniteMagnitude) }
}

public extension Angle {

	/// A textual representation of `Angle`.
    var description: String { return "\(degrees)°" }
}
...
public extension Angle
{
    ...
    static func abs(_ x: Angle) -> Angle
		{ return Angle((Swift.abs as (CGFloat) -> CGFloat)(x.radians)) }
}
...
public func	asin(_ x: Angle.Raw) -> Angle		{ return Angle(asin(x) as CGFloat) }
public func	asinh(_ x: Angle.Raw) -> Angle	{ return Angle(asinh(x) as CGFloat) }
public func	acos(_ x: Angle.Raw) -> Angle		{ return Angle(acos(x) as CGFloat) }
public func	acosh(_ x: Angle.Raw) -> Angle	{ return Angle(acosh(x) as CGFloat) }
public func	atan(_ x: Angle.Raw) -> Angle		{ return Angle(atan(x) as CGFloat) }
public func	atanh(_ x: Angle.Raw) -> Angle	{ return Angle(atanh(x) as CGFloat) }

public func fabs(_ x: Angle) -> Angle					{ return Angle(Swift.abs(x.radians)) }
...
public func nan(_ tag: String) -> Angle					{ return Angle(nan(tag) as Double) }
...
public func		degree(from radian: CGFloat) -> CGFloat
{
...
public func		radian(from degree: CGFloat) -> CGFloat
{
	return degree * .pi / 180
}
