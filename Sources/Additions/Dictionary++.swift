...
extension Dictionary
{
	...
	public init?<S: Sequence>(keysAndValues: S) where S.Iterator.Element == (Key, Value)
	{
		self.init()
	...
	public init<S: Sequence>(merging keysAndValues: S, combine: (Value, Value) throws -> Value) rethrows where S.Iterator.Element == Element
	{
		self.init()
		for (k, v) in keysAndValues
		{
	...
	/// using a combining closure to determine the value for any duplicate keys.
	public mutating func merge<S: Sequence>(contentsOf other: S, combine: (Value, Value) throws -> Value) rethrows where S.Iterator.Element == Element
	{
		for (k, v) in other
		{
			if let oldValue = self[k]
	...
	public func merged<S: Sequence>(with other: S, combine:  (Value, Value) throws -> Value) rethrows -> [Key: Value] where S.Iterator.Element == Element
	{
		var new = self
