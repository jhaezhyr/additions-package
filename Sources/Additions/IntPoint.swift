...
{
	public init(_ intPoint: IntPoint)
	{
		self.init(x: Double(intPoint.x), y: Double(intPoint.y))
	}
}
...
public extension IntPoint
{
	public var CGPoint:	CoreGraphics.CGPoint
		{ return CoreGraphics.CGPoint.init(x: Double(x), y: Double(y)) }
}
